...
int main(void) {
	...
	const float host_result = std::accumulate(h_input.begin(),
		h_input.end(), 0.0f);
	...
	// one additional slot to store the total sum
	float *d_p_sums_and_total = 0;
	cudaMalloc((void**)&d_p_sums_and_total, sizeof(float)
		* (num_blocks + 1));
	// launch one kernel to compute, per-block, a partial sum
	block_sum << <num_blocks, block_size, block_size *
		sizeof(float) >> > (d_input, d_p_sums_and_total,
			num_elements);
	// launch a single block to compute the sum of the partial sums
	block_sum << <1, num_blocks, num_blocks *
		sizeof(float) >> > (d_p_sums_and_total,
			d_p_sums_and_total + num_blocks, num_blocks);

	// copy the result back to the host
	float device_result = 0;
	cudaMemcpy(&device_result, d_p_sums_and_total +
